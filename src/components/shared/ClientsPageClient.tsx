...
import { mockClients, mockUsers, mockAppointments } from "@/lib/mock-data";
...
export function ClientsPageClient() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tab !== "ALL") clients = clients.filter((c) => c.status === tab);
    if (search.trim()) {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">{mockClients.length} clientes cadastradas</p>
        </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex gap-1 border border-border rounded-lg p-1">
          <button
            ...
            className={`p-1.5 rounded transition-colors ${view === "list" ? "bg-muted" : "hover:bg-muted/50"}`}
            title="Lista"
          >
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors ${view === "kanban" ? "bg-muted" : "hover:bg-muted/50"}`}
            title="Kanban"
          >
      ...
      {view === "list" ? (
                  ...
                  return (
                    <tr key={client.id} className="border-b border-border last:border-0 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-4 py-3 text-muted-foreground hidden md:table-cell">
                        {lastAppt
                          ? lastAppt.date.toLocaleDateString("pt-BR")
                          : "Nunca"}
                      </td>
        ...
        <ClientsKanban clients={mockClients} />
      )}
